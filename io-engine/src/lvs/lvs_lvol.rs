...
impl LogicalVolume for Lvol {
    ...
    fn usage(&self) -> LvolSpaceUsage {
        ...
        // if self is snapshot and parent is clone, then do ancestor calculation
        // from clone.
        let clone_source_snap_usage = self.calculate_clone_source_snap_usage();
        unsafe {
            ...
            LvolSpaceUsage {
                ...
                // calculate_clone_source_snap_usage results S1+S2+S3
                allocated_bytes_snapshot_from_clone: clone_source_snap_usage
                    .map(|s| allocated_bytes_snapshots - s),
            }
        }
    }
}
